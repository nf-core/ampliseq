...
- [#NNNN](https://github.com/nf-core/ampliseq/pull/NNNN) - Added `summary_tables/ampliseq.taxonomy.<classifier>.<database>.tsv.gz`, a slim, consistent-schema taxonomy table for every classifier/database actually run (Kraken2 excluded), joined with per-ASV barrnap/decontam/filter annotations (`barrnap_domain`, `decontam_contaminant`/`decontam_not_contaminant`, `passed_ssu_filter`, `passed_length_filter_asv`, `passed_codon_filter`, `passed_length_filter_itsx`, `ampliseq_accept`); also written as Parquet by default, skip with `--skip_parquet_summary` (by @erikrikarddaniel)
