...
- [#1062](https://github.com/nf-core/ampliseq/pull/1062) - Added `--consolidate_taxonomies` (`most-specific` or `score`) to pick a per-ASV winning database across multiple `--dada_ref_taxonomy` databases, instead of always using the first-listed one; the winning result is published as `dada2/ASV_tax.consolidated.<method>.tsv` (by @erikrikarddaniel)
