...
- [#XXXX](https://github.com/nf-core/ampliseq/pull/1070) - VSEARCH LCA now also publishes the raw `--blast6out` hit table (`ASV_tax_vsearch_lca.*.txt` / `ASV_ITS_tax_vsearch_lca.*.txt`) alongside the existing LCA and taxonomy TSV outputs (by @pieterprovoost)
