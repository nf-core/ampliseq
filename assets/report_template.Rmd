...
if ( !isFALSE(params$input_samplesheet) ) {
    # samplesheet input
    cat("\nSequencing data was provided in the samplesheet file `", basename(params$input_samplesheet), "` that is displayed below:", sep="")
    ...
    # fasta input
    cat("\nASV/OTU sequences were provided in the fasta file `", basename(params$input_fasta), "`. ", sep="")
} else if ( !isFALSE(params$input_folder) ) {
    # folder input
    cat("\nSequencing data was retrieved from folder `", params$input_folder, "`. ", sep="")
}
if ( !isFALSE(params$metadata) ) {
    cat("\nMetadata associated with the sequencing data was provided in `", basename(params$metadata), "` and is displayed below:", sep="")
...
if ( params$dada_sample_inference == "independent" ) {
    cat("The ASVs can be found in [`savont/pooled_final_asvs.fasta`](../savont/). And the corresponding",
        ...
        "and per-sample information can be found in folder [savont/per-sample/](../savont/per-sample/). ")
...
if ( !isFALSE(params$cut_dada_ref_taxonomy) ) {
    cut_logs <- strsplit(params$cut_dada_ref_taxonomy, " ")[[1]]
    cut_stat <- function(lines, pattern) trimws(sub("^[^:]*:", "", grep(pattern, lines, value = TRUE, fixed = TRUE)[1]))
    cut_dada_ref_taxonomy <- do.call(rbind, lapply(cut_logs, function(log) {
        lines <- readLines(log)
        data.frame(
            database = sub("^assignTaxonomy_(.*)\\.cutadapt\\.log$", "\\1", basename(log)),
            orig     = cut_stat(lines, "Total reads processed:"),
            filt     = cut_stat(lines, "Reads written (passing filters):"),
            orig_bp  = cut_stat(lines, "Total basepairs processed:"),
            filt_bp  = cut_stat(lines, "Total written (filtered):")
        )
    }))
    cut_dada_ref_taxonomy <- cut_dada_ref_taxonomy[order(cut_dada_ref_taxonomy$database), ]

    if (nrow(cut_dada_ref_taxonomy) == 1) {
        with(cut_dada_ref_taxonomy, cat("The taxonomic reference database was cut by primer sequences to improve matching.
            The original database had ",orig," sequences with ",orig_bp,
            ", retained were ",filt," sequences that represented ",filt_bp,".\n\n",
            sep = ""))
        cut_dada_ref_taxonomy_filt <- cut_dada_ref_taxonomy$filt
    } else {
        cat("The taxonomic reference databases were cut by primer sequences to improve matching:\n\n")
        with(cut_dada_ref_taxonomy, cat(paste0("- `", database, "`: the original database had ", orig, " sequences with ", orig_bp,
            ", retained were ", filt, " sequences that represented ", filt_bp, ".\n"), "\n", sep = ""))
        cut_dada_ref_taxonomy_filt <- with(cut_dada_ref_taxonomy, paste0(filt, " (", database, ")", collapse = ", "))
    }
}
...
# consolidated taxonomy records the winning database per ASV
if ("database" %in% colnames(asv_tax)) {
    dada2_db_counts <- table(asv_tax$database[asv_tax$database != "" & !is.na(asv_tax$database)])
    dada2_db_counts_str <- paste0("`", names(dada2_db_counts), "` (", dada2_db_counts, " ASVs)", collapse = ", ")
    cat("The taxonomy of each ASV was taken from the database that won consolidation: ", dada2_db_counts_str, ".\n\n", sep = "")
}

# Calculate the classified numbers/percent of asv
non_rank_cols <- colnames(asv_tax) %in% c("ASV_ID", "confidence", "sequence", "database") | grepl("_confidence$", colnames(asv_tax))
...
# They all root at a domain, whichever prefix they write it with -- Greengenes 85 uses "k__".
...
if ( params$sintax_cutoff != "" && params$sintax_cutoff != "0.8" ) {
    cat("A SINTAX confidence score threshold of",params$sintax_cutoff,"was applied for taxonomic classifications.\n\n")
}
...
if ( !isFALSE(params$dada2_taxonomy) ) {
    ...
    if ( exists("dada2_db_counts_str") ) {
        methods_taxonomic_classification_dada <- paste0(methods_taxonomic_classification_dada,
            ", taking each ASV's taxonomy from the database that won consolidation: ", dada2_db_counts_str)
    }
...
cat("in folder [pipeline_info](../pipeline_info) file `nf_core_ampliseq_software_mqc_versions.yml`.")
```
...
including software versions collected at runtime in file `nf_core_ampliseq_software_mqc_versions.yml` (can be viewed with a text editor),
