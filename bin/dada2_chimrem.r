...
write.table(nochim2[,1:(length(nochim2)-1)], file = opt$table, sep = "\t", row.names = F, quote = F, col.names = c("#ASV_ID", colnames(nochim2[2:(length(nochim2)-1)])))
...
write.table(nochim2, file = opt$reltable, sep = "\t", row.names = F, quote = F, col.names = c("#ASV_ID", colnames(nochim2[2:length(nochim2)])))
