...
# Some tables needed below
asvtax <- merge(taxonomy, asvs, by.x = 'ASV_ID', by.y = 'ASV_ID')
asvtable <- asvtax[c('ASV_ID', colnames(asvs)[2:(n_samples+1)], 'sequence', colnames(taxonomy)[2:(ncol(taxonomy)-1)])]

# Create list of tables, most of them empty, to be written to xlsx format
xlo <- list(
  'dataset' = data.frame(datasetID = character(), provider_email = character()),
  'event' = data.frame(
...
write.xlsx(xlo, file = 'SBDI.xlsx', as.table = TRUE)
