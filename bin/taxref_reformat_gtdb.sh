...
sed '/^>/s/>\([^ ]\+\) \([^[]\+\) \[.*/>\2(\1\)/' ar122*.fna bac120*.fna | sed 's/[a-z]__//g' | sed 's/ /_/g' > gtdb_assignTaxonomy.fna

# Write the assignTaxonomy() fasta file: gtdb_addSpecies.fna
sed '/^>/s/>\([^ ]\+\) .*;s__\([^[]\+\) \[.*/>\1 \2/' ar122*.fna bac120*.fna > gtdb_addSpecies.fna
