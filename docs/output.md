...
Alternatively, ASVs can be taxonomically classified using the SINTAX algorithm as implemented in VSEARCH. Depending on the reference taxonomy database (specified with `sintax_ref_taxonomy` or a user file with `sintax_ref_tax_custom`), sequences can be classified down to species rank when the reference supports it.
