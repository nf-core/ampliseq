...
### ASV inferrence with Savont

[Savont](https://doi.org/10.64898/2026.05.26.727271) generates Amplicon Sequence Variants (ASVs) at single-nucleotide resolution from long-read amplicon sequencing data such as Oxford Nanopore (ONT) R10.4 sequencing (preferably with SUP basecalling) or PacBio HiFi sequencing.
...
`--dada_ref_taxonomy` accepts a comma-separated list of databases (e.g. `--dada_ref_taxonomy gtdb,silva`) to classify against several databases at once; one full set of the output files below is published per listed database. By default, only the **first-listed** database feeds every downstream step that expects a single taxonomy (QIIME2 filtering, diversity, barplots, ANCOM, R objects). Use `--consolidate_taxonomies most-specific` or `--consolidate_taxonomies score` to instead pick a winning database per ASV across all listed databases -- `most-specific` picks whichever database resolved the deepest rank for that ASV, `score` picks whichever database reported the highest assignTaxonomy bootstrap confidence for that ASV (both break ties by database order in `--dada_ref_taxonomy`). When set, the winning per-ASV result is published as `dada2/ASV_tax.consolidated.<method>.tsv`, with an added `database` column recording which listed database each row's winning classification came from. Consolidation is currently DADA2-only -- it does not compare results across different classification methods (e.g. DADA2 vs. SINTAX).
...
- `pipeline_info/`
  ...
  - Reports generated by the pipeline: `pipeline_report.html`, `pipeline_report.txt` and `nf_core_ampliseq_software_mqc_versions.yml`. The `pipeline_report*` files will only be present if the `--email` / `--email_on_fail` parameter's are used when running the pipeline.
