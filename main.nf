...
def helpMessage() {
	...
	Main arguments:
	  ...
	  --reads [folder]              Folder containing paired-end demultiplexed fastq files
	  ...
	  --metadata [file]				Path to metadata sheet, when missing most downstream analysis are skipped (barplots, PCoA plots, ...). 
                                    Tab separated file with metadata column headers.
	  --manifest [file]     		Path to manifest.tsv table with the following labels in this exact order: sampleID, forwardReads, reverseReads.
	...
	References:                     If you have trained a compatible classifier before
	  --classifier [file]   		Path to QIIME2 classifier file (typically *-classifier.qza)
