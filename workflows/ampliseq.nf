...
workflow AMPLISEQ {
	...
	*/
	...
	if ( find_truncation_values ) {
		...
		ch_trunc.subscribe { 
			if ( "${it[0][1]}".toInteger() + "${it[1][1]}".toInteger() <= 10 ) { log.warn "`--trunclenf` was set to ${it[0][1]} and `--trunclenr` to ${it[1][1]}, this is too low! Please either increase `--trunc_qmin`, decrease `--trunc_qmin`, or set `--trunclenf` and `--trunclenr`." }
			else if ( "${it[0][1]}".toInteger() <= 10 ) { log.warn "`--trunclenf` was set to ${it[0][1]}, this is too low! Please either increase `--trunc_qmin`, decrease `--trunc_qmin`, or set `--trunclenf` and `--trunclenr`." }
			else if ( "${it[1][1]}".toInteger() <= 10 ) { log.warn "`--trunclenr` was set to ${it[1][1]}, this is too low! Please either increase `--trunc_qmin`, decrease `--trunc_qmin`, or set `--trunclenf` and `--trunclenr`." }
			else log.warn "Probably everything is fine, but this is a reminder that `--trunclenf` was set automatically to ${it[0][1]} and `--trunclenr` to ${it[1][1]}. If this doesnt seem reasonable, then please change `--trunc_qmin`, `--trunc_qmin`, or set `--trunclenf` and `--trunclenr` directly."
			}
	} else { 
