...
is_fast_input = WorkflowAmpliseq.checkIfFileHasExtension( params.input.toString().toLowerCase(), fasta_extensions )
...
if ( !single_end && !params.illumina_pe_its && (params.trunclenf == false || params.trunclenr == false) && !is_fast_input ) {
...
workflow AMPLISEQ {
    ...
    //
    PARSE_INPUT ( params.input, is_fast_input, single_end, params.multiple_sequencing_runs, params.extension )
